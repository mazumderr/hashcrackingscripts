...
for line in passes:
    ...
    # Extract the algorithm and make sure it is MD5
    if ((line[index1+1:index2]) != "MD5"):
        print ("Error: only input MD5-based hashes")
        sys.exit(1)

    # Extract the hash, remove the trailing carriage return, and do the
    # Base64 decoding.
    hashes.append((line[index2+1:].strip('\n')))
...
for word in words:
    ...
    newhash = hashlib.md5(word.strip('\n').encode()).hexdigest()
    #print(newhash)
    # print word.strip('\n') + " " + newhash
